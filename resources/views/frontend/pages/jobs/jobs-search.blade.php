            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="row">
                            ...
                            @foreach ($all_jobs as $data)
                                <div class="col-lg-12">
                                    <div class="single-job-list-item">
                                        <span class="job_type"><i class="far fa-clock"></i>
                                            {{ str_replace('_', ' ', $data->employment_status) }}</span>
                                        <a
                                            href="{{ route('frontend.jobs.single', ['id' => $data->id, 'any' => Str::slug($data->title)]) }}">
                                            <h3 class="title">{{ $data->title }}</h3>
                                        </a>
                                        <span class="company_name"><strong>{{ 'Company:' }}</strong>
                                            {{ $data->company_name }}</span>
                                        <span class="deadline"><strong>{{ 'Deadline:' }}</strong>
                                            {{ date('d M Y', strtotime($data->deadline)) }}</span>
                                        <ul class="jobs-meta">
                        ...
                        @else
                            <div class="col-lg-12">
                                <div class="alert alert-warning d-block">{{ 'No Job Posts Found' }}</div>
                            </div>
                        @endif
                    </div>
                    <div class="col-lg-12">
