...
if (!function_exists('active_menu_frontend')) {
...
if (!function_exists('sendSubscriberEmail')) {
...
if (!function_exists('sendEmail')) {
    function sendEmail($to, $name, $subject, $message, $from = '')
    {
        ...
        $message = str_replace("@company", 'UMDAA Health Care', $message);
...
if (!function_exists('sendPlanEmail')) {
...
if (!function_exists('single_post_share')) {
    function single_post_share($url, $title, $img_url)
    {
        ...
        $twitter_share_link = 'https://twitter.com/intent/tweet?text=' . $post_title . '&amp;url=' . $encoded_url . '&amp;via=' . 'UMDAA Health Care';
        $linkedin_share_link = 'https://www.linkedin.com/shareArticle?mini=true&url=' . $encoded_url . '&amp;title=' . $post_title;
        $pinterest_share_link = 'https://wa.me/?text=' . $post_title.'%0A%0A'.$encoded_url;

        $output .= '<li><a class="facebook" href="' . $facebook_share_link . '"><i class="fab fa-facebook-square"></i></a></li>';
        $output .= '<li><a class="twitter" href="' . $twitter_share_link . '"><i class="fab fa-twitter-square"></i></a></li>';
        $output .= '<li><a class="linkedin" href="' . $linkedin_share_link . '"><i class="fab fa-linkedin"></i></a></li>';
        $output .= '<li><a class="pinterest" href="' . $pinterest_share_link . '"><i class="fab fa-whatsapp-square"></i></a></li>';
...
if (!function_exists('get_work_category_by_id')) {
...
if (!function_exists('get_child_menu_count')) {
...
if (!function_exists('minify_css_lines')) {
...
if (!function_exists('google_captcha_check')) {
    function google_captcha_check($token)
    {
        $captcha_url = 'https://www.google.com/recaptcha/api/siteverify';
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $captcha_url);
        curl_setopt($curl, CURLOPT_POST, 1);
        curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query(array('secret' => '6Ld5wHMaAAAAABLRohYknvhnu0r04N1jmuKUJVqo', 'response' => $token)));
...
if (!function_exists('get_field_by_type')) {
    function get_field_by_type($type, $name, $placeholder, $options = [], $required = null, $mimes = null)
    {
        $markup = '';
        $required_markup_html = 'required="required"';
        switch ($type) {
        case('email'):
            $required_markup = !empty($required) ? $required_markup_html : '';
        ...
        case('password'):
            $required_markup = !empty($required) ? $required_markup_html : '';
            $markup = ' <div class="form-group"> <input type="password" id="' . $name . '" name="' . $name . '" class="form-control" placeholder="' . __($placeholder) . '" ' . $required_markup . '></div>';
            break;
        case('url'):
            $required_markup = !empty($required) ? $required_markup_html : '';
...
if (!function_exists('get_topic_name_by_id')) {
 ...
 */
if (!function_exists('delete_dir_with_file')) {
...
if (!function_exists('chmod_file_folder')) {
...
if (!function_exists('copy_dir_with_files')) {
...
if (!function_exists('render_background_image_markup_by_attachment_id')) {
...
if (!function_exists('render_image_markup_by_attachment_id')) {
